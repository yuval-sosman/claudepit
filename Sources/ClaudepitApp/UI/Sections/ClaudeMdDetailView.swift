...
struct ClaudeMdDetailView: View {
    ...
    private func acceptImprovement() {
        ...
        do {
            try improved.write(to: file.path, atomically: true, encoding: .utf8)
            content = improved
            pendingImproved = nil
            onChanged?()
        } catch {
    ...
    private func loadContent() {
        ...
        Task.detached(priority: .userInitiated) {
            ...
            await MainActor.run {
                content = result ?? ""
                loadError = result == nil ? "Could not read file" : nil
            }
        }
    }
}
